...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<div class="pagination flex-center flex">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<span class="flex flex-center active"><a href="#">' . $page . '</a></span>';
            } else {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, 'class="left-arrow icon"') . $links;

            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, 'class="right-arrow icon"');
        }
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        if (strlen($text)<5){
        	$text = $page;
        	return
                '<span class="flex flex-center"><a  href="' . $currentURI . $this->index . $page . '">'.$text.'</a></span>';
        }            
        return
                '<span class="flex flex-center"><a '.$text.' href="' . $currentURI . $this->index . $page . '"></a></span>';
    }
